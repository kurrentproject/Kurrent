...
#if defined(NDEBUG)
# error "Kurrent cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlockTest = uint256("0x6df4d9e709cfb430a1a1e650b676bb589bd57ba5244fec685a1660a545298bce");
uint256 hashGenesisBlockMain = uint256("0x8e4bd83026896b9e882d4970b1d63bf6785371b5d6595ea3c905497e54f4ad09");
uint256 hashGenesisBlock = hashGenesisBlockMain;

uint256 hashGenesisMerkleRootTest = uint256("0xa79c870a60e9b37f4d5691a7d926067a2c20c9292a996289019ef3e79f6129c9");
uint256 hashGenesisMerkleRootMain = uint256("0xf6587dca41cb0f3bd057d0eef8f2ffbb17b18d14f993685f7e5c246ce92838d9");
uint256 hashGenesisMerkleRoot = hashGenesisMerkleRootMain;

CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kurrent: starting difficulty is 1 / 2^20
...
int64 CTransaction::nMinTxFee = 1000; // 0.00001 Kurrent
/** Fees smaller than this (in ztoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 1000; // 0.00001 Kurrent
...
const string strMessageMagic = "Kurrent Signed Message:\n";
...
{
    // Kurrent: IsDust() detection disabled, allows any valid dust to be relayed.
                    ...
                    if (txout.nValue == libzerocoin::ZQ_LOVELACE * COIN)
                    {
                        ...
                        BOOST_FOREACH(const CTxIn& txin, vin) {
                            ...
                            if (txin.scriptSig.IsZerocoinSpend()) {
								...
								int pubcoinId_s = (int)pubcoinId;

                                // VERIFY COINSPEND TX
                                unsigned int countPubcoin = 0;
                                BOOST_FOREACH(const CZerocoinEntry& pubCoinItem, listPubCoin) {

                                    if (pubCoinItem.denomination == libzerocoin::ZQ_GOLDWASSER && pubCoinItem.id == pubcoinId_s && pubCoinItem.nHeight != -1) {
                                ...
                                if(!passVerify){
                                    ...
                                    BOOST_REVERSE_FOREACH(const CZerocoinEntry& pubCoinItem, listPubCoin) {
                                        ...
                                        if (pubCoinItem.denomination == libzerocoin::ZQ_GOLDWASSER && pubCoinItem.id == pubcoinId_s && pubCoinItem.nHeight != -1) {
                                            //printf("## denomination = %d, id = %d, pubcoinId = %d height = %d\n", pubCoinItem.denomination, pubCoinItem.id, pubcoinId_s, pubCoinItem.nHeight);
                                            libzerocoin::PublicCoin pubCoinTemp(ZCParams, pubCoinItem.value, libzerocoin::ZQ_GOLDWASSER);
                                            if (!pubCoinTemp.validate()) {
                                                return state.DoS(100, error("CTransaction::CheckTransaction() : Error: Public Coin for Accumulator is not valid !!!"));
                                            }
                        ...
                        BOOST_FOREACH(const CTxIn& txin, vin) {
                            ...
                            if (txin.scriptSig.IsZerocoinSpend()) {
                                ...
                                int pubcoinId_s = (int)txin.nSequence;

                                // VERIFY COINSPEND TX
                                unsigned int countPubcoin = 0;
                                BOOST_FOREACH(const CZerocoinEntry& pubCoinItem, listPubCoin) {

                                    if (pubCoinItem.denomination == libzerocoin::ZQ_RACKOFF && pubCoinItem.id == pubcoinId_s && pubCoinItem.nHeight != -1) {
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        // There is a free transaction area in blocks created by most min-ers,
...
static const int64 MinimumCoin = 1 * COIN;
static const int SubsidyHalvingInterval = 1051200;
...
int64 GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if (nHeight <= 0)
		...
		return 11400000 * COIN;
		
	std::string cseed_str = prevHash.ToString().substr(13, 7);
	const char* cseed = cseed_str.c_str();
	long seed = hex2long(cseed);
	int rand = GenerateMTRandom(seed, 64);

	int64 nSubsidy = (64 + rand) * COIN;
...
static const int64 nRetargetTimespan = nTargetSpacing; 

static const int64 nLookbackInterval = 3; // 3 blocks lookback for difficulty adjustment
static const int64 nLookbackTimespan = nLookbackInterval * nTargetSpacing; 

static const int64 LimUp = nLookbackTimespan * 100 / 120; // 20% up
static const int64 LimDown = nLookbackTimespan * 120 / 100; // 20% down
...
int64 GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	if (pindex->nHeight < 3)
		return 0;
    ...
    for (unsigned int i=0; i<vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && !tx.IsZerocoinSpend())
        {
            ...
            if (fStrictPayToScriptHash)
            {
                ...
                // this is to prevent a "rogue" from creating
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot, bool isVerifyDB) const
{
    ...
    // First transaction must be coinbase, second could be also
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
		...
		for (unsigned i = 1; i < vtx.size(); i++)
			...
			{
				if (nBonusReward <= 0)
					return state.DoS(100, error("AcceptBlock() : more than one coinbase"));

				CBlock prevBlock;
				prevBlock.ReadFromDisk(pindexPrev);
				if (prevBlock.vtx[0].vout.size() != vtx[i].vout.size())
					return state.DoS(100, error("AcceptBlock() : incorrect bonus block reward tx"));
		...
		if (nBonusReward > 0)
			return state.DoS(100, error("AcceptBlock() : missing bonus block reward tx"));
...
bool InitBlockIndex() {	
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Kurrent: first X11 coin with zerocoin feature !!";
        CTransaction txNew;
        ...
        txNew.vout[0].nValue = GetBlockValue(0, 0, 0);
        ...
        block.nVersion = 2;
        block.nTime    = 1479764382;
        block.nBits    = startBits;
        block.nNonce   = 3525712;
		...
		if (true && (block.GetHash() != hashGenesisBlock)) {
			...
			while (Hash9(BEGIN(block.nVersion), END(block.nNonce)) > hashTarget)
			{
				++block.nNonce;
				printf("nNonce = %u\n", block.nNonce);
				if (block.nNonce == 0)
...
unsigned char pchMessageStart[4] = { 0xdb, 0xc3, 0xfa, 0xfc };
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
	...
	int64 nBonusReward = GetProofOfWorkBonusRewardFactor(pindexBest);
	if (nBonusReward > 0)
	{
		CBlock prevBlock;
		prevBlock.ReadFromDisk(pindexBest);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
		...
		{
            return error("KurrentMiner : generated block is stale"); 
		}
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KurrentMiner : ProcessBlock, block not accepted");
    }
...
void static KurrentMiner(CWallet *pwallet)
{
    printf("KurrentMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Kurrent-Miner");
    ...
    try { loop {
        ...
        printf("Running KurrentMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            loop
            {
				...
				if (thash == 0)
				{
					printf("KurrentMiner() : Out of memory\n");
					throw std::runtime_error("KurrentMiner() : Out of memory");
				}
    ...
    {
        printf("KurrentMiner terminated\n");
        throw;
    }
	catch (std::exception &e)
	{
        printf("KurrentMiner error\n");
	}
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KurrentMiner, pwallet));
}
