...
int AddressTableModel::InputZeroCoinAmount(QWidget* parent, int flag)
{
	QStringList items;
	...
	if (flag == 1) {
		item = QInputDialog::getItem(parent,
			tr("Choose amount to mint"), tr("Amount:"),
			items, 0, false, &ok);
	}
	else {
		item = QInputDialog::getItem(parent,
			tr("Choose amount to spend"), tr("Amount:"),
			items, 0, false, &ok);
	}
...
{
	int amt = InputZeroCoinAmount(parent, 1);
	if (amt <= 0)
		return false;
...
{
	int amt = InputZeroCoinAmount(parent, 2);
	if (amt <= 0)
		return false;
